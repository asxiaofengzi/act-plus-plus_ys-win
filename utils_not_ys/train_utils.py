...
from ys_policy import ACTPolicy, CNNMLPPolicy, DiffusionPolicy
import os
import numpy as np
import torch
import csv
import pickle
from utils_ys.misc_utils import *
...
def forward_pass(data, policy, opts):
...
def train_bc(train_dataloader, val_dataloader, config):
    ...
    validation_perf_file_path = config['validation_perf_file_path']
    eval_perf_file_path = config['eval_perf_file_path']
    opts={
        'gpu':config['gpu']
    }
    ...
    for step in tqdm(range(num_steps + 1)):
        ...
        if step % validate_every == 0:
            ...
            with torch.inference_mode():
                ...
                for batch_idx, data in enumerate(val_dataloader):
                    print(f"forwarding batch {batch_idx}")
                    forward_dict = forward_pass(data, policy,opts)
                    validation_dicts.append(forward_dict)
                    if (batch_idx+1) >= config['num_val_batches']:
                ...
                if epoch_val_loss < min_val_loss:
                    ...
                    best_policy=deepcopy(policy.serialize())
                    best_ckpt_info = (step, min_val_loss, best_policy)
                    ckpt_path = os.path.join(ckpt_dir, f'best_policy_seed_{seed}.ckpt')
                    torch.save(best_policy, ckpt_path)
            ...
            print(f'best val loss {min_val_loss:.6f} at step {step}')
            append_to_csv(validation_perf_file_path, [step,round(epoch_val_loss,3)])
            summary_string = ''
        ...
        if (step > 0) and (step % eval_every == 0):
            ...
            ckpt_name = f'policy_last_seed_{seed}.ckpt'
            ckpt_path = os.path.join(ckpt_dir, ckpt_name)
            torch.save(policy.serialize(), ckpt_path)
            print('saved current policy')
            success, avg_return = eval_bc(config, ckpt_name, save_episode=True, num_rollouts=10)
            print('finished evaluating')
            if log_to_wandb:
                wandb.log({'success': success}, step=step)
            append_to_csv(eval_perf_file_path, [step,success,avg_return])

        # training
        print('performing train step ...')
        policy.train()
        print('finished policy.train()')
        optimizer.zero_grad()
    ...
    print(f'Training finished:\nSeed {seed}, best val loss {min_val_loss:.6f} at step {best_step}')
